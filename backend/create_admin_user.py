...
def create_admin_user():
    ...
    with get_db_session() as db:
        try:
            ...
            existing_admin = db.query(User).filter(User.email == "lcjadmin9676@lcj.com").first()
            if existing_admin:
            ...
            admin_user = User(
                id=uuid.uuid4(),
                email="lcjadmin9676@lcj.com",
                username="admin",
                password_hash=get_password_hash("lcjadmin@9676"),
                is_active=True,
...
def create_special_user():
    ...
    with get_db_session() as db:
        try:
            # Check if admin user already exists
            existing_admin = db.query(User).filter(User.email == "testmilan1381@gmail.com").first()
            if existing_admin:
                print("Admin user already exists!")
                print(f"Email: {existing_admin.email}")
                print(f"Role: {existing_admin.role}")
            ...
            admin_user = User(
                id=uuid.uuid4(),
                email="testmilan1381@gmail.com",
                username="Milan",
                password_hash=get_password_hash("testmilan@1381"),
                is_active=True,
                is_verified=True,  # Skip email verification for admin
                role="user",
                providers=["password"]
